...
class HysteresisTriangle(Module):
    states = None

    def __init__(self, h_data, size=1.0, trainable=True):
        # note h_values inside class are normalized in range [0-1]

        super(HysteresisTriangle, self).__init__()
        self.h_data = h_data
        self.old_h = torch.empty(1).to(self.h_data)

        # generate mesh in normalized space
        self.mesh_points = torch.tensor(create_mesh(size))
        self.vector_shape = len(self.mesh_points)
        hyst_density_vector = torch.ones(self.mesh_points.shape[0]).to(self.h_data)
    ...
    def get_states(self, h: torch.Tensor):
        ...
        n_mesh_points = self.vector_shape

        # list of hysteresis states with initial state set
        states = torch.empty((len(hs), n_mesh_points)).to(self.h_data)
        states[0] = torch.ones(n_mesh_points).to(self.h_data) * -1.0
        ...
        for i in range(1, len(hs)):
            if hs[i] > hs[i - 1]:
                ...
                states[i] = states[i - 1].to(self.h_data)
    ...
    def predict_magnetization(self,
        ...
        dens = hyst_density_vector

        m = torch.sum(dens * states[1:], dim=-1) / len(dens)
        return s*m + o
