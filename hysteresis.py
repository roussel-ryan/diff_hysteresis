...
class Hysteresis(Module):
    states = None
    dtype = torch.float64
    old_h = torch.empty(1, dtype=dtype)
    ...
    def __init__(self, h_data, h_min, h_max, b_sat, n, trainable=True):
        ...
        self._vector_shape = int(n ** 2 / 2 + n / 2)
        hyst_density_vector = torch.ones(self._vector_shape,
                                         dtype=self.dtype)
    ...
    def set_density_vector(self, vector):
        assert vector.shape[0] == self._vector_shape, f'{vector.shape[0]} ' \
                                                      f'vs. {self._vector_shape} '
    ...
    def get_states(self, h: torch.Tensor):
        ...
        states[0] = torch.ones(self._xx.shape) * -1
    ...
    def predict_magnetization(self,
        ...
        if h_new is not None:
            ...
            # don't recalcuate states if the new h is not equal to the old h
        ...
        m = torch.empty(len(h))  # m is the resulting magnetization
        dens = utils.vector_to_tril(hyst_density_vector, self._n)
